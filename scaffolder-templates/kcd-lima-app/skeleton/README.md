...
![Screenshot of the game](https://raw.githubusercontent.com/KCD LimaInc/software-templates/master/scaffolder-templates/kcd-lima-app/skeleton/docs/board.png)
...
```bash
oc new-project demo --description='kcd-lima-app'
```
*You can choose a different name, but then you have to update `argo/kcd-lima-app.yaml` before you deploy the app (see below in the **Flow** section).*
...
Fork and clone GitOps [manifests repo](https://github.com/redhat-developer-demos/kcd-lima-app-manifests)


```bash
git clone https://github.com/<yourgithubuser>/kcd-lima-app-manifests
cd kcd-lima-app-manifests
```
...
Update all references to quay.io with your repos for kcd-lima-app references:

```bash
sed -i 's/rhdevelopers/yourquayuser/g' k8s/deployment.yaml
sed -i 's/redhat-developer-demos/yourgithubuser/g' argo/kcd-lima-app.yaml
git add .
...
```bash
oc apply -f argo/kcd-lima-app.yaml
```
...
### Delete deployed  app
```bash
oc delete all -l app.kubernetes.io/name=kcd-lima-app
```
